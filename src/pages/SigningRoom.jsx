...
const SigningRoom = () => {
  ...
  const isReviewStatus = (value) => {
    ...
    return value.toLowerCase().includes('review')
  }
  ...
  const formatVerificationRemaining = (verifiedAt) => {
    if (!verifiedAt) return null
    const remainingMs = new Date(verifiedAt).getTime() + VERIFICATION_TTL_MS - Date.now()
  ...
  const isVerificationExpired = (signerData) => {
    if (!signerData?.verified_at) return false
    if (!signerData.document?.requires_identity_verification || !signerData.requires_verification) return false
    if (signerData.status === 'SIGNED') return false

    const verifiedAt = new Date(signerData.verified_at).getTime()
    return Date.now() - verifiedAt > VERIFICATION_TTL_MS
  }

  const handleVerificationExpiry = async (signerData) => {
    if (!signerData?.id) return
    if (signerData.status === 'SIGNED') return
    ...
    try {
      ...
      setShowSigningModal(false)
      setError('Tu verificación venció. Vuelve a verificar tu identidad para firmar.')
      setFlowState(FLOW_STATES.NEEDS_VERIFICATION)
    } catch (err) {
      console.error('Error expiring verification:', err)
    }
  }

  const ensureVerificationIsValid = async (signerData) => {
    if (!isVerificationExpired(signerData)) return true
    await handleVerificationExpiry(signerData)
    return false
  }
  ...
  const redeemContinuityToken = async (token) => {
    try {
      ...
      if (response?.ok) {
        ...
        newSearchParams.delete('continuity_token')
  ...
  const loadSignerData = async () => {
    try {
      ...
      const signerData = response?.signer
      const documentData = response?.document

      if (!signerData || !documentData) {
      ...
      // 2. Check if verification TTL expired
      if (isVerificationExpired(signerData)) {
        await handleVerificationExpiry(signerData)
        return
      }

      // 3. Check if document is expired (optional)
      ...
      // 4. Determine flow state based on signer and document status
      ...
      if (signerData.status === 'SIGNED') {
        ...
        if (signerData.status === 'VERIFIED') {
          ...
          await loadVerificationAttempt(signerData.id)
  ...
  }, [signer?.verified_at, signer?.status, signer?.requires_verification, signer?.document?.requires_identity_verification])

  const loadVerificationAttempt = async (signerId) => {
    try {
      const response = await callSigningRoom({
  ...
  const handleStartVerification = async () => {
    try {
      ...
      if (signer?.verified_at && isVerificationExpired(signer)) {
        const response = await callSigningRoom({
          action: 'get-latest-attempt',
          signingToken: transactionId,
          signerId: signer.id
        ...
        const lastAttempt = response?.attempt

        if (lastAttempt?.didit_session_id) {
          try {
            await DiditService.deleteSession(lastAttempt.didit_session_id)
            await callSigningRoom({
              action: 'expire-attempt',
              signingToken: transactionId,
              signerId: signer.id,
              attemptId: lastAttempt.id
            })
          } catch (deleteError) {
            console.warn('Didit session delete failed:', deleteError)
          }
        }
      }
  ...
  const progressPercentage = isSigned ? 100 : (isReadyToSign || isOrderBlocked || isVerifying || isInReview || isReviewApproved) ? 50 : 0
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded ${
                          ...
                          {isReadyToSign ? 'Habilitada' : isOrderBlocked ? 'Bloqueada' : isSigned ? 'Completada' : 'Pendiente'}
                        </span>
                        <span className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-100 text-slate-600">
                          <img
                      ...
                      <div className={`rounded-md p-3 mb-3 ${
                        ...
                        {isReadyToSign && signer?.verified_at && (
                          <p className="mt-1 text-[11px] text-emerald-700">
                            Tienes {formatVerificationRemaining(signer.verified_at)} para firmar.
                          </p>
                        )}
                      </div>
                      ...
                      {isReadyToSign && (
                        <div className="space-y-2">
                          {signingToken ? (
                            <button
                              onClick={async () => {
                                const canOpen = await ensureVerificationIsValid(signer)
                                if (canOpen) setShowSigningModal(true)
                              }}
